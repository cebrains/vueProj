<template>
    <footer class="footer-area" :style="{height:footerHeight}" v-if="options.length||optionType=='ImageOptions'">
        <div class="optionsWrap" >
            <FooterSingleOpt v-if="options.length==1 && optionType=='TextOptions'" />
            <FooterPicker v-else-if="options.length>0 && optionType=='TextOptions'"/>
            <FooterScroll v-else-if="optionType=='ImageOptions'"/>
        </div>
    </footer>
</template>
